...
// swapped firebase for your AuthContext
import { useAuth } from '@/contexts/AuthContext';

// Importing Feather icons
import { FiBriefcase, FiFileText, FiHeart, FiUser, FiLogOut, FiMenu, FiX } from 'react-icons/fi';
...
export default function CandidateSidebar() {
  ...
  useEffect(() => {
    if (user) {
      // Try common name fields, fall back to email or username
      const name = (user.name as string) || `${user.first_name ?? ''} ${user.last_name ?? ''}`.trim() || user.email || 'User Name';
      const initials = name
        .split(' ')
        .filter(Boolean)
        .map((n: string) => n[0])
      ...
      // user role/position fallback
      const role = (user.position as string) || (user.role as string) || 'Position';
      setUserRole(role);

      // user id - try common id fields
      const uid = user.id ?? user._id ?? user.uid ?? null;
      setUserId(uid);
      ...
      // if not logged in, reset to defaults
      setUserInitials('JD');
  ...
  const handleNavClick = (href: string) => {
    // Open profile pages in new tab
    if (href.includes('/candidate_profile/') || href.includes('/profile/')) {
  ...
  const navItems = [
    ...
    },
    {
      ...
      icon: (
        <svg className={styles.navIcon} viewBox="0 0 24 24" fill="none">
          <path d="M20 7H4C2.89543 7 2 7.89543 2 9V19C2 20.1046 2.89543 21 4 21H20C21.1046 21 22 20.1046 22 19V9C22 7.89543 21.1046 7 20 7Z" stroke="#4F46E5" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
          <path d="M16 21V5C16 4.46957 15.7893 3.96086 15.4142 3.58579C15.0391 3.21071 14.5304 3 14 3H10C9.46957 3 8.96086 3.21071 8.58579 3.58579C8.21071 3.96086 8 4.46957 8 5V21" stroke="#4F46E5" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      )
    },
    {
    ...
    },
    {
      id: 'profile',
      text: 'My Profile',
      href: userId ? `/candidate_profile/${userId}` : '#',
      icon: (
        <svg className={styles.navIcon} viewBox="0 0 24 24" fill="none">
      ...
      >
        ...
        <div className={styles.bottomSection}>
          ...
          <div
            ...
            onClick={() => handleNavClick(userId ? `/profile/${userId}` : '#')}
