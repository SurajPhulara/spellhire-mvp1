// components/employer/jobs/JobCard.tsx
"use client";
...
export default function JobCard({ job, onStatusUpdate, onDelete }: JobCardProps) {
  ...
  const handleStatusUpdate = async (newStatus: JobStatus) => {
    setIsUpdating(true);
    try {
      const response = await JobService.updateJobStatus(job.id, { new_status: newStatus });
      if (response.success) {
  ...
  return (
    <>
      <style jsx>{`
        ...
        .trackButton:hover:not(:disabled) {
          transform: translateY(-2px);
          box-shadow: 0 4px 12px rgba(102, 126, 234, 0.4);
        }
        ...
        .statusButton.activate {
          background: #d1fae5;
          color: #065f46;
          border: 1px solid #6ee7b7;
        }

        .statusButton.activate:hover:not(:disabled) {
          background: #a7f3d0;
        }
      ...
      <div className="jobCard">
        ...
        <div className="cardHeader">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
            <div className="companyInfo">
              <div className="companyInitial">
                {job.title.charAt(0).toUpperCase()}
              </div>
              <div style={{ flex: 1 }}>
                <h3 className="jobTitle">{job.title}</h3>
                <p className="companyName">{job.category || 'General'}</p>
              </div>
            </div>
            <div className="headerActions">
              ...
              <span className={`statusBadge ${getStatusBadgeClass(job.status)}`}>
                {job.status}
              </span>
            </div>
          </div>
        ...
        <div className="jobDetails">
          <div className="detailItem">
            <svg className="detailIcon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
            </svg>
            <span>{JobService.formatLocation(job.location)} {job.work_mode === 'REMOTE' && '(Remote)'}</span>
          </div>
          <div className="detailItem">
            <svg className="detailIcon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
            </svg>
            <span>{job.job_type?.replace('_', ' ')} • {job.experience_level}</span>
          </div>
          <div className="detailItem">
            <svg className="detailIcon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="skillsContainer">
            ...
            <div className="skillsList">
              ...
              {job.required_skills.length > 5 && (
                <span className="moreSkills">+{job.required_skills.length - 5} more</span>
              )}
            </div>
          </div>
        )}
        ...
        <div style={{ padding: '20px 24px', borderBottom: '1px solid #f3f4f6', background: '#fafafa' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
            <div style={{ textAlign: 'center' }}>
              ...
              <div style={{ fontSize: '12px', color: '#6b7280', fontWeight: '600' }}>Views</div>
            </div>
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: '24px', fontWeight: '700', color: '#111827', marginBottom: '4px' }}>
                {job.application_count}
              </div>
              <div style={{ fontSize: '12px', color: '#6b7280', fontWeight: '600' }}>Applications</div>
            </div>
          </div>
        </div>
        ...
        <div className="cardFooter">
          <div className="jobActions">
            ...
            {(job.status === JobStatus.ACTIVE || job.status === JobStatus.PAUSED) && (
              <button
                onClick={(e) => { e.stopPropagation(); handleStatusUpdate(job.status === JobStatus.ACTIVE ? JobStatus.PAUSED : JobStatus.ACTIVE); }}
                disabled={isUpdating}
                className={`statusButton ${job.status === JobStatus.PAUSED ? 'activate' : ''}`}
              >
                {isUpdating ? 'Updating...' : job.status === JobStatus.ACTIVE ? (
                  <>
                    <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {(job.status === JobStatus.ACTIVE || job.status === JobStatus.PAUSED) && (
              <button
                onClick={(e) => { e.stopPropagation(); handleStatusUpdate(JobStatus.CLOSED); }}
                disabled={isUpdating}
            ...
            {job.status === JobStatus.CLOSED && (
              <button
                onClick={(e) => { e.stopPropagation(); handleStatusUpdate(JobStatus.ACTIVE); }}
                disabled={isUpdating}
              ...
              <div className="shareOptions">
                ...
                <div className="socialButtons">
                  ...
                  <button
                    ...
                    className="socialButton whatsapp"
                  >
                    <svg viewBox="0 0 24 24" fill="currentColor">
                ...
                <div className="copyLinkSection">
                  ...
                  <div className="copyLinkContainer">
                    ...
                    <button
                      ...
                      className={`copyLinkButton ${copySuccess ? 'copySuccess' : ''}`}
                    >
